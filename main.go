...
func buildGoogleUrls(searchTerm, countryCode, languageCode string, pages, count int) ([]string, error) {
	...
	if googleBase, found := googleURLS[countryCode]; found {
		for i := 0; i < pages; i++ {
			start := i * count
...
func gglQuery(searchTerm, countryCode, languageCode string, proxyString interface{}, pages, count, backoff int) ([]SearchResult, error) {
	results := []SearchResult{}
	resultCounter := 0
...
func scrapeClientRequest(searchURL string, proxyString interface{}) (*http.Response, error) {
	...
	req.Header.Set("User-Agent", getUAString())
...
func main() {
	r := mux.NewRouter()
	r.HandleFunc("/", nil)
	http.Handle("/", csrf.Protect([]byte(nil))(r))
	var queryString string
	fmt.Println("Please enter search string ")
	fmt.Scanln(&queryString)
	res, err := gglQuery(queryString, "in", "en", nil, 1, 30, 10)
	var colorFlag bool
	yellow := "\033[33m"
	blue := "\033[34m"
	if err == nil {
		for _, res := range res {
			if colorFlag {
				colorFlag = false
				fmt.Println(blue, res, "\033[0m")

			} else {
				colorFlag = true
				fmt.Println(yellow, res, "\033[0m")
